...
clear()
print(Fore.BLUE + "Pilot's Seat" + Fore.GREEN + " v1.2")
time.sleep(fakeCPU * 2)
...
print(
    Fore.YELLOW +
    "You hardly remember anything from training, but you recognize the three numbers as computer commands..."
)
time.sleep(fakeCPU)
print(Fore.YELLOW +
      "101, Check status\n301, Open the door\nand 502, is to abort.")
...
def codeInput():
	...
	if landingCount == goal:
		...
		print(
		    Fore.GREEN +
		    "Your journey has come to an end, you are satsified with your exploration of the universe!"
		)
		time.sleep(fakeCPU * 4)
		endGame()
	emergency = 0
	...
	if locList[currentLoc][2] == 1:
		if fuelLvl >= locList[currentLoc][3]:
			...
			for i in range(locList[currentLoc][3], 0, -1):
				print(Fore.CYAN + "\rAltitude : " + Fore.WHITE + str(i),
				      end='  ')
				time.sleep(fakeCPU)
				fuelLvl -= 1
			print(Fore.CYAN + "\nLanding Procedure Completed")
			orbiting = 0
			if reland == 1:
				if revisitCount > 0:
					revisitCount -= 1
...
def prog3():  #Program 202, Ascent Program
	...
	if fuelLvl >= locList[currentLoc][3]:
		...
		for i in range(0, locList[currentLoc][3], 1):
			print(Fore.CYAN + "\rAltitude : " + Fore.WHITE + str(i), end='  ')
			time.sleep(fakeCPU)
			fuelLvl -= 1
		print(Fore.CYAN + "\nAscent Procedure Completed")
		orbiting = 1
	else:
		print(
		    Fore.YELLOW +
...
def prog7():  #Program #402, Go to Target
	...
	for i in range(abs(locList[int(target)][1] - locList[int(currentLoc)][1])):
		if abs(locList[int(target)][1] -
			...
			break
		print(Fore.MAGENTA + "\rDistance to Target: " + Fore.WHITE + str(
		    (abs(locList[int(target)][1] - locList[int(currentLoc)][1]) - i)),
		      end='  ')
		fuelLvl -= 1
		time.sleep(fakeCPU)
...
def domum():
	...
	global revisitCount
	print(Fore.YELLOW +
	      "After landing on Domum, you are in a safe environment.")
	time.sleep(fakeCPU)
	print(Fore.YELLOW + "The cities of Domum remind you of the past.")
	time.sleep(fakeCPU)
	print(
	    Fore.YELLOW +
	    "Domum has a Fuelling station nearby, but takes some time to extract fuel from the depleted soil."
	)
	time.sleep(fakeCPU)
	if revisitCount == 0:
		fuelLvl = fuelMax
		...
		print(
		    Fore.GREEN +
		    "Domum's local Fuel station has refueled your spacecraft completely."
		)
	else:
		print(
		    Fore.GREEN +
		    "Domum's local Fuel station cannot refuel your spacecraft yet.\nCheck back after visiting different locations "
		    + Fore.WHITE + str(revisitCount) + Fore.GREEN + " more times.")
	codeInput()
